...
export default function DropdownFull() {
  ...
  return (
    <Menu as="div" className="relative inline-flex w-full">
      {({ open }) => (
        <>
          <MenuButton className="btn w-full justify-between min-w-[11rem] bg-white border-gray-200 hover:border-gray-300 text-gray-600 hover:text-gray-800" aria-label="Select option">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <MenuItems className="font-medium text-sm text-gray-600 divide-y divide-gray-200 focus:outline-hidden">
              {options.map((option, optionIndex) => (
                <MenuItem key={optionIndex}>
                  {({ active }) => (
                    <button
                      className={`flex items-center justify-between w-full py-2 px-3 cursor-pointer ${active ? 'bg-gray-50' : ''} ${option.id === selected && 'text-violet-500'}`}
