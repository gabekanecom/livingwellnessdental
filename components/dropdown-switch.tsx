      ...
      <Menu as="div" className="relative">
        <MenuButton className="grow flex items-center truncate">
          ...
          <div className="truncate">
            <span className="text-sm font-medium group-hover:text-gray-800">{options[selected].channel}</span>
          </div>
        ...
        <Transition
          as="div"
          className="origin-top-right z-10 absolute top-full left-0 min-w-[15rem] bg-white border border-gray-200 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1"
          ...
          <MenuItems as="ul" className="focus:outline-hidden">
            {options.map((option, optionIndex) => (
              <MenuItem key={optionIndex} as="li">
                {({ active }) => (
                  <button
                    className={`w-full font-medium text-sm block py-1.5 px-3 ${active ? 'text-gray-800' : 'text-gray-600'}`}
