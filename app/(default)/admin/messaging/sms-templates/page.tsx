...
export default function SmsTemplatesPage() {
  ...
  const fetchTemplates = async () => {
    try {
      const res = await fetch('/api/admin/messaging/sms-templates');
      if (res.ok) {
  ...
  const createTemplate = async () => {
    if (!newTemplate.name || !newTemplate.slug || !newTemplate.content) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/admin/messaging/sms-templates/${created.id}`);
      } else {
  ...
  const deleteTemplate = async (template: SmsTemplate) => {
    ...
    try {
      const res = await fetch(`/api/admin/messaging/sms-templates/${template.id}`, {
        method: 'DELETE',
      });
  ...
  const getSegmentCount = (content: string) => {
    ...
    if (len <= 160) return 1;
    return Math.ceil(len / 153); // Multipart messages use 153 chars per segment
  };
                  ...
                  <tr key={template.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap hidden md:table-cell">
                      <div className="text-sm">
                        <span className={`font-medium ${getCharCount(template.content) > 160 ? 'text-amber-600' : 'text-gray-900'}`}>
                          {getCharCount(template.content)}
                        </span>
                        <span className="text-gray-500"> chars</span>
                        {getSegmentCount(template.content) > 1 && (
                          <span className="text-xs text-gray-400 ml-1">
                            ({getSegmentCount(template.content)} seg)
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          href={`/admin/messaging/sms-templates/${template.id}`}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Hello {{name}}, your appointment is tomorrow at {{time}}."
                  />
                  <div className="flex justify-between mt-1">
                    <p className="text-xs text-gray-500">Use {'{{variable}}'} for dynamic content</p>
                    <p className={`text-xs ${newTemplate.content.length > 160 ? 'text-amber-600' : 'text-gray-500'}`}>
                      {newTemplate.content.length}/160 chars
                    </p>
                  </div>
                </div>
