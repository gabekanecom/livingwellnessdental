...
export default function MailItem({ mail }: { mail: Mail }) {
  ...
  return (
    <div className="py-6">
        ...
        <div className="grow">
          <div className="sm:flex items-start justify-between mb-0.5">
            ...
            <div className="xl:flex items-center mb-2 sm:mb-0">
              <button className="text-sm font-semibold text-gray-800 text-left truncate" onClick={() => setOpen(!open)}>{mail.name}</button>
              {open &&
                <>
      ...
      {/* Body */}
      {open &&
        <div className="text-sm text-gray-800 mt-4 space-y-2" dangerouslySetInnerHTML={{ __html: mail.message }}></div>
      }
    </div>
  )
}